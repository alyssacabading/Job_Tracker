...
router.put('/:id?', validateJobData(true), async (req, res) => {  // uses middleware to validate incoming job data
    try {
        const jobId = req.params.id;
...
router.delete('/:jobId/contacts', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Contact removed from job successfully', updatedJob });
